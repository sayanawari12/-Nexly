import React from 'react';
import { Code2 } from 'lucide-react';
import { motion } from 'framer-motion';
import '../../styles/sections.css';

const Footer = () => {
  const socialLinks = [
    {
      name: 'GitHub',
      icon: (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4" />
          <path d="M9 18c-4.51 2-5-2-7-2" />
        </svg>
      ),
      url: 'https://github.com/sayanawari12',
      label: 'GitHub profile'
    },
    {
      name: 'LinkedIn',
      icon: (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z" />
          <rect width="4" height="12" x="2" y="9" />
          <circle cx="4" cy="4" r="2" />
        </svg>
      ),
      url: 'https://linkedin.com',
      label: 'LinkedIn profile'
    },
    {
      name: 'Instagram',
      icon: (
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <rect width="20" height="20" x="2" y="2" rx="5" ry="5" />
          <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" />
          <line x1="17.5" x2="17.51" y1="6.5" y2="6.5" />
        </svg>
      ),
      url: 'https://instagram.com',
      label: 'Instagram profile'
    }
  ];

  return (
    <footer className="footer-branding-section" id="footer">
      <div className="footer-container">
        {/* Soft Radial Purple Glow Effect */}
        <div className="footer-glow-effect" aria-hidden="true" />

        {/* 1. Website Logo & Title */}
        <div className="footer-logo-brand">
          <div className="footer-logo-icon-box">
            <Code2 size={24} className="footer-code-icon" />
          </div>
          <span className="footer-brand-name">
            BCA <span className="footer-brand-badge">DEPARTMENT WEBSITE</span>
          </span>
        </div>

        {/* 2. Platform Tagline */}
        <p className="footer-tagline">
          Learning. Building. Growing.
        </p>

        {/* 3. Developer Credit */}
        <div className="footer-developer-credit">
          <span className="credit-label">Designed & Developed by</span>
          <span className="developer-name">Sayan Awari</span>
        </div>

        {/* 4. Social Links */}
        <div className="footer-social-links-wrapper">
          {socialLinks.map((social) => (
            <motion.a
              key={social.name}
              href={social.url}
              target="_blank"
              rel="noopener noreferrer"
              className="footer-social-btn"
              aria-label={social.label}
              whileHover={{ scale: 1.08, y: -3 }}
              whileTap={{ scale: 0.95 }}
            >
              <span className="social-btn-icon">{social.icon}</span>
              <span className="social-btn-text">{social.name}</span>
            </motion.a>
          ))}
        </div>

        {/* 5. Divider Line */}
        <div className="footer-divider-line" />

        {/* 6. Footer Copyright */}
        <p className="footer-copyright-text">
          © {new Date().getFullYear()} BCA Department Website. All Rights Reserved.
        </p>
      </div>
    </footer>
  );
};

export default Footer;
