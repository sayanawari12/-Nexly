...
async function runTests() {
    ...
    try {
        ...
        const userLogin = await axios_1.default.post(`${API_URL}/auth/login`, {
            identifier: userUsername,
            password: userPassword,
        });
        const body = userLogin.data;
    ...
    const socketNoToken = (0, socket_io_client_1.io)(SOCKET_URL, { autoConnect: false });
    ...
    const socketBadToken = (0, socket_io_client_1.io)(SOCKET_URL, {
    ...
    const clientSocket = (0, socket_io_client_1.io)(SOCKET_URL, {
...
`;
    let submissionId = '';
    try {
        const subRes = await axios_1.default.post(`${API_URL}/submissions`, {
            problemId,
            languageId,
            sourceCode: pythonCode,
        }, { headers: { Authorization: `Bearer ${userToken}` } });
        submissionId = subRes.data.data.id;
    ...
    try {
        ...
        const otherLogin = await axios_1.default.post(`${API_URL}/auth/login`, {
            identifier: otherUser,
            password: userPassword,
        });
        otherToken = otherLogin.data.data.accessToken;
    }
    catch (err) { }
    const intruderSocket = (0, socket_io_client_1.io)(SOCKET_URL, {
