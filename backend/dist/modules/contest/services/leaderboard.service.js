...
class LeaderboardService {
     ...
     */
    encodeScore(solvedCount, totalPoints, totalPenalty, scoringType, lastSubmissionTimeMs = 0) {
        if (scoringType === client_1.ScoringType.ICPC) {
     ...
     */
    async updateScore(contestId, userId, username, solvedCount, totalPoints, totalPenalty, problemDetails, scoringType, lastSubmissionTimeMs = 0) {
     ...
     */
    async getLeaderboard(contestId, status, isAdmin = false) {
        const isFrozenState = status === client_1.ContestStatus.FROZEN || status === client_1.ContestStatus.ENDED;
     ...
     */
    async saveSnapshot(contestId, isFrozen) {
        const rankings = await this.getLeaderboard(contestId, isFrozen ? client_1.ContestStatus.LIVE : client_1.ContestStatus.ENDED, true);
     ...
     */
    async rebuildLeaderboard(contestId) {
        ...
        for (const score of scores) {
            ...
            try {
                const details = score.problemDetails;
            ...
            const redisScore = this.encodeScore(score.solvedCount, score.totalPoints, score.totalPenalty, contest.scoringType, lastSubmissionTimeMs);
