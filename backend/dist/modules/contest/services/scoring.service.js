...
class ScoringService {
     ...
     */
    calculateIcpcScore(contest, submissions) {
        ...
        for (const problemId of Object.keys(subsByProblem)) {
            const subs = subsByProblem[problemId].sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
            let solved = false;
            let attempts = 0;
            let penalty = 0;
            let firstSolvedAt;
            for (const sub of subs) {
                ...
                if (sub.status === client_1.SubmissionStatus.ACCEPTED) {
                    solved = true;
                ...
                else if (sub.status === client_1.SubmissionStatus.WRONG_ANSWER ||
                    sub.status === client_1.SubmissionStatus.TIME_LIMIT_EXCEEDED ||
                    sub.status === client_1.SubmissionStatus.MEMORY_LIMIT_EXCEEDED ||
                    sub.status === client_1.SubmissionStatus.RUNTIME_ERROR) {
                    attempts++;
                }
            }
     ...
     */
    calculateIoiScore(contest, submissions, problemPointsWeights) {
        const contestStartMs = new Date(contest.startTime).getTime();
        const contestEndMs = new Date(contest.endTime).getTime();
        // Group submissions by problem
        const subsByProblem = {};
