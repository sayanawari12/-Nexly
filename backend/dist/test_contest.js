...
async function runContestTests() {
    ...
    try {
        // User A
        const regA = await axios_1.default.post(`${API_URL}/auth/register`, {
            email: userAEmail,
            username: userAUsername,
            password,
        });
        userAId = regA.data.data.id;
        const loginA = await axios_1.default.post(`${API_URL}/auth/login`, {
            identifier: userAUsername,
            password,
        });
        userAToken = loginA.data.data.accessToken;
        // User B
    ...
    let languageId = '';
    try {
        const createRes = await axios_1.default.post(`${API_URL}/problems`, {
        ...
        await axios_1.default.post(`${API_URL}/problems/${problemId}/testcases/batch`, {
            testCases: [
    ...
    try {
        const createContestRes = await axios_1.default.post(`${API_URL}/contests`, {
        ...
        contestId = createContestRes.data.data.id;
    ...
    try {
        await axios_1.default.get(`${API_URL}/contests/${contestId}`, {
    ...
    try {
        await axios_1.default.post(`${API_URL}/contests/${contestId}/register`, {}, {
            headers: { Authorization: `Bearer ${userAToken}` },
        });
        await axios_1.default.post(`${API_URL}/contests/${contestId}/register`, {}, {
    ...
    const socketA = (0, socket_io_client_1.io)(SOCKET_URL, { auth: { token: userAToken } });
    const socketB = (0, socket_io_client_1.io)(SOCKET_URL, { auth: { token: userBToken } });
...
`;
    try {
        await axios_1.default.post(`${API_URL}/submissions`, {
            problemId,
            languageId,
...
`;
    ...
    try {
        const leaderboardRes = await axios_1.default.get(`${API_URL}/contests/${contestId}/leaderboard`, {
            headers: { Authorization: `Bearer ${userAToken}` },
        });
        const standings = leaderboardRes.data.data;
    ...
    try {
        const frozenLeaderboardRes = await axios_1.default.get(`${API_URL}/contests/${contestId}/leaderboard`, {
            headers: { Authorization: `Bearer ${userAToken}` }, // Normal user token
        });
        const frozenStandings = frozenLeaderboardRes.data.data;
        console.log('Public Standings (during Freeze):');
        frozenStandings.forEach((entry) => {
            console.log(` - Rank ${entry.rank}: User: ${entry.username}, Solved: ${entry.solvedCount}, Penalty: ${entry.totalPenalty}m`);
        });
        // Retrieve admin leaderboard (should return live rankings, i.e. User B's score updated)
        const adminLeaderboardRes = await axios_1.default.get(`${API_URL}/contests/${contestId}/leaderboard`, {
            headers: { Authorization: `Bearer ${adminToken}` }, // Admin token
        });
        const adminStandings = adminLeaderboardRes.data.data;
        console.log('Admin Standings (during Freeze):');
        adminStandings.forEach((entry) => {
            console.log(` - Rank ${entry.rank}: User: ${entry.username}, Solved: ${entry.solvedCount}, Penalty: ${entry.totalPenalty}m`);
        });
    ...
    try {
        await axios_1.default.post(`${API_URL}/contests/${contestId}/unfreeze`, {}, {
            headers: { Authorization: `Bearer ${adminToken}` },
        });
    ...
    try {
        const unfrozenLeaderboardRes = await axios_1.default.get(`${API_URL}/contests/${contestId}/leaderboard`, {
            headers: { Authorization: `Bearer ${userAToken}` },
        });
        const finalStandings = unfrozenLeaderboardRes.data.data;
    ...
    try {
        ...
        await axios_1.default.delete(`${API_URL}/problems/${problemId}`, {
            headers: { Authorization: `Bearer ${adminToken}` },
        });
